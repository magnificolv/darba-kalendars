...
async function callClaude(messages, maxTokens = 10000) {
  const res = await fetch(API_URL, {
    ...
    body: JSON.stringify({ model: "claude-sonnet-4-5-20250929", max_tokens: maxTokens, messages }),
  });
...
function lookupTour(tours, id) {
  ...
  for (const key of Object.keys(tours)) {
    const kClean = key.replace(/[\s/]/g, "").toLowerCase();
    if (kClean === idClean || kClean === nClean) return tours[key];
  }
  return null;
}
  ...
  const buildCalendar = async () => {
    ...
    try {
      ...
      for (let d = 1; d <= dim; d++) {
        ...
        if (e?.tour && e.tour !== "b") {
          needed.add(e.tour);
          const n = normalizeTour(e.tour);
          if (n !== e.tour) needed.add(n);
        }
      }

      setLoadStep(`📄 Meklē ${needed.size} tūr${needed.size === 1 ? 'i' : 'es'} PDF failos...`); setLoadPct(20);
...
Some tours have multiple day-pattern variants (e.g. one row for "1-5d" and another for "6,7d") — include each variant.
...
Return ONLY valid JSON, no other text:
{
  "tours": {
    ...
    "7117": [{"dayPattern":"1-5d","startTime":"13:48","endTime":"23:06","hours":9.30}]
  }
}`
      });

      setLoadPct(50);
      const raw = await callClaude([{ role: "user", content: pdfMsgs }], 12000);
      setLoadPct(80);
